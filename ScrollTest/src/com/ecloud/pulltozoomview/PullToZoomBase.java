...
public abstract class PullToZoomBase<T extends View> extends LinearLayout
	...
	private void init(Context context, AttributeSet attrs) {
		...
		if (attrs != null) {
			...
			isParallax = a.getBoolean(
					R.styleable.PullToZoomListView_isHeadParallax, true);
	...
	public boolean onInterceptTouchEvent(MotionEvent event) {
		...
		case MotionEvent.ACTION_DOWN: {
			Log.e("----", "isReadyForPullStart");
			if (isReadyForPullStart()) {
				Log.e("----", "isReadyForPullStart+down");
	...
	public boolean onTouchEvent(@NonNull MotionEvent event) {
		...
		case MotionEvent.ACTION_UP: {
			...
			if (mIsBeingDragged) {
				Log.e("----", "onTouchEven))))))))up");
				mIsBeingDragged = false;
				// If we're already refreshing, just scroll back to the top
				if (isZooming()) {
					Log.e("----", "onTouchEvent++++++++isZooming");
					smoothScrollToTop();
